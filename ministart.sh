...
# Bare-bones startup script for hosts with no GPU and no local Ollama server.
# Starts only the API (FastAPI/uvicorn) and UI (Streamlit). For the full
# stack including Ollama, use ./newstart.sh instead.
...
# Environment knobs that can be overridden before running `ministart.sh`:
...
#  * PUBLIC_IP: force the public URL host shown at the end (skips auto-detect)
...
wait_for_port() {
  ...
  for i in $(seq 1 "${attempts}"); do
    if curl --max-time 2 -sf "${url}" >/dev/null 2>&1 || curl --max-time 2 -sf "${url}/health" >/dev/null 2>&1; then
...
# Check required commands (no Ollama check - GPU/Ollama-free mode)
...
# nccl, ...) even though this box has no GPU. Pre-install the much smaller
...
start_log_monitor() {
  ...
  nohup bash -c "
    tail -n +1 -F '${API_LOG}' '${UI_LOG}' 2>/dev/null \
...
echo ""
echo "═══════════════════════════════════════════════════════════════"
color_echo green "🎯 Services started! (mini mode: no GPU, no Ollama)"
echo ""
...
color_echo yellow "💡 To stop all services, run: pkill -f 'ministart.sh|uvicorn|streamlit'"
echo ""
