...
def _compose_lightweight_reply(payload: ChatRequest, retrieved: List[Dict[str, Any]], mode: str) -> str:
    ...
    if not retrieved:
        # No RAG data - provide banking definitions if applicable (format with bullet points)
        if is_banking_terms and is_definition_question:
            ...
            for term in ["pd", "dti", "ltv", "ndi", "fmv"]:
                if term in question_lower and term in banking_defs:
                    definitions.append(banking_defs[term])
            if definitions:
                response = "Here are the banking term definitions:\n\n"
                for defn in definitions:
                    response += f"- {defn}\n"
                response += "\n*Note: This information comes from general banking knowledge. Upload relevant documents to enhance the knowledge base.*"
                return response
        
        return (
            f"I couldn't find specific information about '{payload.message}' in the knowledge base.\n\n"
            "**Suggestions:**\n"
            "- Upload relevant documents to enhance the knowledge base\n"
    ...
    # If we have clean answers, use them - format with bullet points
    if clean_answers:
        # Format as summary + bullet points
        summary = f"Based on the knowledge base, here's the answer to '{payload.message}':"
        response = f"{summary}\n\n"
        for i, answer in enumerate(clean_answers[:3], 1):
            answer_clean = answer.strip().rstrip('.!?')
            response += f"- {answer_clean}\n"
        # Add banking definitions if question asks for them and they're not in the answer
        if is_banking_terms and is_definition_question:
            for term, definition in banking_defs.items():
                if term in question_lower and term.lower() not in response.lower():
                    response += f"\n- {definition}"
        return response
    ...
    if cleaned_snippets:
        # Format as summary + bullet points
        summary = f"Here's relevant information from the knowledge base:"
        response = f"{summary}\n\n"
        for snippet in cleaned_snippets[:2]:
            snippet_clean = snippet.strip()[:400]  # Limit length
            response += f"- {snippet_clean}\n"
        # Add banking definitions if relevant
        if is_banking_terms and is_definition_question:
            for term, definition in banking_defs.items():
    ...
    # Final fallback with banking definitions if applicable - format with bullet points
    if is_banking_terms and is_definition_question:
        definitions = []
        for term in ["pd", "dti", "ltv", "ndi", "fmv"]:
            if term in question_lower and term in banking_defs:
        ...
        if definitions:
            ...
            for defn in definitions:
                response += f"- {defn}\n"
            response += "\n*Based on general banking knowledge. The retrieved context may contain additional relevant information.*"
            return response
    
    return (
        f"I found {len(retrieved)} document(s) related to '{payload.message}', but couldn't extract a clear answer.\n\n"
        "**Suggestions:**\n"
        "- Try rephrasing your question\n"
...
def chat_endpoint(payload: ChatRequest) -> ChatResponse:
    ...
    # TEMPORARY STRATEGY: Use only model knowledge base (RAG disabled)
    # 1. Generate answer from LLM's knowledge base only
    # 2. Skip RAG retrieval entirely
    
    # Generate answer from LLM knowledge base only
    conversation_history = _build_conversation_context(payload.history if hasattr(payload, 'history') else [])
    initial_answer = None
    rag_time = 0
    tfidf_time = 0
    ...
    if USE_OLLAMA and model_to_use:
        try:
            llm_start = time.time()
            # Generate answer from LLM's knowledge base only (no RAG)
            initial_answer = _generate_gemma_fallback(payload, mode, model_to_use, conversation_history)
            llm_time = (time.time() - llm_start) * 1000
            if initial_answer and len(initial_answer.strip()) > 30:
                # Check if it's not just a generic response
                generic_phrases = ["i'm a banking", "i can help", "please try rephrasing", "i couldn't find"]
                if not any(phrase in initial_answer.lower() for phrase in generic_phrases):
                    logger.info("Generated answer from LLM knowledge base (RAG disabled)")
                else:
                    logger.warning("LLM returned generic response, treating as failed")
                    initial_answer = None
            else:
                logger.warning(f"LLM returned short/empty response: {len(initial_answer) if initial_answer else 0} chars")
                initial_answer = None
        except Exception as exc:
            llm_time = 0
    ...
    # Use only model knowledge base (skip RAG entirely)
    if initial_answer:
        # Use LLM knowledge base answer only (RAG disabled)
        logger.info("Using LLM knowledge base answer only (RAG disabled)")
        confidence, confidence_score = _get_confidence_level(0.0, "general_knowledge")
        source_type = "general_knowledge"
        
        reply_text = initial_answer
        reply_text = _format_response_with_structure(reply_text, mode, confidence)
        reply_text += "\n\n*Answer based on model knowledge base.*"
        
        retrieved = []
        related_questions = _generate_related_questions(payload.message, mode, [])
    else:
        # No LLM answer - retry with stronger prompt
        logger.info("No initial LLM answer - retrying with enhanced prompt")
        confidence, confidence_score = _get_confidence_level(0.0, "general_knowledge")
        source_type = "general_knowledge"
        
        # Retry with a more direct, forceful prompt
        if USE_OLLAMA and model_to_use:
            try:
                llm_start = time.time()
                # Create a more direct prompt that forces the model to answer
                direct_prompt = f"""You are a helpful AI assistant. Answer this question directly:

Question: {payload.message}

Instructions:
- DO NOT say "I'm a banking AI assistant" or "I can help with..."
- DO provide the actual answer using your knowledge
...
Answer:"""
                ...
                resp = requests.post(
                    ...
                    json={
                        ...
                        "stream": False,
                        "options": {"temperature": 0.3, "top_p": 0.9, "num_predict": 2000, "num_ctx": 4000},
                    },
                    timeout=60,
                )
                resp.raise_for_status()
                data = resp.json()
                fallback_reply = data.get("response") or data.get("data") or ""
                llm_time = (time.time() - llm_start) * 1000
                
                if fallback_reply and len(fallback_reply.strip()) > 30:
                    # Check again for generic responses
                    generic_phrases = ["i'm a banking", "i can help", "please try rephrasing", "i couldn't find", "i don't have"]
                    if not any(phrase in fallback_reply.lower() for phrase in generic_phrases):
                        reply_text = fallback_reply.strip()
                        reply_text = _format_response_with_structure(reply_text, mode, confidence)
                        reply_text += "\n\n*Answer based on model knowledge base.*"
                    else:
                        # Still generic - provide a helpful message
                        logger.warning("Model still returning generic responses after retry")
                        reply_text = f"I apologize, but I'm having difficulty generating a specific answer to your question about '{payload.message}'. Please try:\n\n- Rephrasing your question\n- Being more specific\n- Breaking it into smaller parts"
                        reply_text = _format_response_with_structure(reply_text, mode, confidence)
                else:
                    reply_text = f"I apologize, but I couldn't generate a complete answer. Please try rephrasing your question: '{payload.message}'"
                    reply_text = _format_response_with_structure(reply_text, mode, confidence)
            except Exception as exc:
                logger.debug("Retry LLM generation failed: %s", exc)
                reply_text = f"I apologize, but I encountered an error while trying to answer your question. Please ensure the Ollama service is running and try again."
                reply_text = _format_response_with_structure(reply_text, mode, confidence)
        else:
            reply_text = "I apologize, but the LLM service is unavailable at this time. Please ensure the Ollama service is running."
            reply_text = _format_response_with_structure(reply_text, mode, confidence)
